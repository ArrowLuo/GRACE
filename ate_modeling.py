...
class BertForSequenceLabeling(PreTrainedBertModel):
    """
    """
    def __init__(self, config, num_labels, task_config):
    ...
    def _get_logits(self, input_ids, token_type_ids, attention_mask, bool_input_embedding=False):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask,
                                       output_all_encoded_layers=False, bool_input_embedding=bool_input_embedding)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None,
        ...
        logits = self._get_logits(input_ids, token_type_ids, attention_mask)
        ...
        if labels is not None:
            ...
            if self.task_config["use_ghl"]:
                weights, self.acc_sum, weight_gradient, weight_gradient_labels \
                    = self.calculate_ce_gradient_weight(logits, labels, attention_mask, self.num_labels,
                                                        self.acc_sum, weight_gradient, weight_gradient_labels)
                weights_label = weights.view(-1, self.num_labels)
                # weights is dynamic with multi-gpu
                loss = self.weighted_ce_loss_fct(logits.view(-1, self.num_labels), labels.view(-1), weights_label)
            else:
                loss = self.ce_loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
    ...
    def vat_loss(self, input_ids, token_type_ids, attention_mask):
        ...
        with torch.no_grad():
            _pred_logits = self._get_logits(input_ids, token_type_ids, attention_mask)
        ...
        with _disable_tracking_bn_stats(self):
            ...
            for _ in range(self.ip):
                ...
                pred_hat = self._get_logits(words_embeddings_ + xi_d, token_type_ids, attention_mask,
                                            bool_input_embedding=True)
                logp_hat_i = F.log_softmax(pred_hat, dim=2).view(-1, self.num_labels)
                pred_i = pred.view(-1, self.num_labels)
            ...
            pred_hat = self._get_logits(words_embeddings_+r_adv, token_type_ids, attention_mask,
                                            bool_input_embedding=True)
            logp_hat_i = F.log_softmax(pred_hat, dim=2).view(-1, self.num_labels)
            pred_i = pred.view(-1, self.num_labels)
